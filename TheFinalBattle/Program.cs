namespace TheFinalBattle
{
    public class Program
    {
        public static void Main()
        {
            ...
            Party heroes = new Party(new HumanPlayer(), new List<Character> { new TrueProgrammer(new Sword()) }, new List<Item> { new HealthPotion(), new HealthPotion(), new HealthPotion() });
            Party monsters_1 = new Party(new ComputerPlayer(), new List<Character> { new Skeleton(new Dagger()) }, new List<Item> { new HealthPotion() });
            Party monsters_2 = new Party(new ComputerPlayer(), new List<Character> { new Skeleton(), new Skeleton() }, new List<Item> { new HealthPotion(), new Dagger(), new Dagger() });
